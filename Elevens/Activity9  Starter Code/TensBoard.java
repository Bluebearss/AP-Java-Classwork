...
public class TensBoard extends Board {
	...
	private boolean containsPairSum10(List<Integer> selectedCards) {
		for (int i = 0; i < selectedCards.size() - 1; i++)
		{
			...
			{
				if (cardAt(selectedCards.get(i).intValue()).pointValue() + cardAt(selectedCards.get(j).intValue()).pointValue() == 10)
				{
					return true;
				}
			}
	...
	private boolean containsQuartet(List<Integer> selectedCards) {
		...
		for (Integer ndx : selectedCards)
		{
			String r = cardAt(ndx.intValue()).rank();
		...
		return tenCount == 4 || jackCount == 4 || queenCount == 4 || kingCount == 4;
	}
}
